...
writeFileSync(`_zod-fromSchema/mega-schema.json`, JSON.stringify(dataSchema, null, "\t"));

const all = [...dataSchema, ...metaSchema];

writeFileSync(`_zod-fromSchema/index.ts`, `// This file is generated by scripts/build-zod.js\n\n`);
...
all.forEach((obj) => {
	...
	format(fullcode, { parser: "typescript" }).then((formatted) => {
		writeFileSync(`_zod-fromSchema/${name}.ts`, formatted);
		console.log(`Wrote ${name}.ts`);

		appendFileSync(`_zod-fromSchema/index.ts`, `export { ${name} } from "./${name}.js"\n`);
	});
});
