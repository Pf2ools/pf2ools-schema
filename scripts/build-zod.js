...
writeFileSync(`_generated-zod/mega-schema.json`, JSON.stringify(dataSchema, null, "\t"));

const all = [...dataSchema, ...metaSchema];

writeFileSync(`_generated-zod/index.ts`, `// This file is generated by scripts/build-zod.js\n\n`);
...
all.forEach((obj) => {
	...
	format(fullcode, { parser: "typescript" }).then((formatted) => {
		writeFileSync(`_generated-zod/${name}.ts`, formatted);
		console.log(`Wrote ${name}.ts`);

		appendFileSync(`_generated-zod/index.ts`, `export { ${name} } from "./${name}.js"\n`);
	});
});
