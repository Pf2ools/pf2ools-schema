...
export const ID = z
    ...
    .regex(/^[a-z0-9][a-z0-9-]*[a-z0-9]$/i, "Use alphanumeric characters only (minimum length 2), along with hyphens that aren't in first or last position")
...
export const sourceData = z
    ...
    copyright: entries
        .describe("Any additional copyright text associated with the source. For instance, the copyright notice that accompanies the OGLv1.0a license would be entered here (but not the license itself). Formatting may be used where it doesn't detract from its interpretation.")
        .optional(),
    authors: z
        .array(z
        .string()
    ...
    converters: z
        ...
        .min(2))
        .describe("A list of unqiue Pf2ools contributors who converted this source. If multiple converters exist, choose one 'principal' to go first in the case of questions or bug reports. This is particularly used for homebrew, both for attributing work done and to help organise updates.")
        .nonempty()
        .refine(...uniqueStrings)
        .optional(),
})
